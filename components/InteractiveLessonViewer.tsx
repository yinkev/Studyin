...
export default function InteractiveLessonViewer({ lesson, learnerId = 'local-dev', onLessonComplete }: InteractiveLessonViewerProps) {
  ...
  // Track lesson accuracy for XP calculation
  ...
  const handleSubmit = useCallback(async () => {
    ...
    // Track accuracy for lesson completion
    ...
    if (correct) {
      const isFast = durationMs < 5000; // Fast answer = under 5 seconds
  ...
  const handleContinue = useCallback(() => {
    ...
    if (activeIndex < mcqs.length - 1) {
      ...
      // Award XP for lesson completion (E2)
      const accuracy = totalAnswered > 0 ? correctAnswers / totalAnswered : 0;
      const isPerfect = accuracy === 1.0; // 100% accuracy
